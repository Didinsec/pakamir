...
var kuhong = db.get('ojan'); // jan diubah
	...
	console.log('WELCOME TO DidinSec API!') // boleh diubah
}

var creatorList = ['didinxc']; // Nama Lu Ngab
var creator = creatorList[Math.floor(Math.random() * creatorList.length)]; // Ini jan diubah
var key = 'didinxc' // Apikey Lu Ngab
...
// Api Features :
        ...
        var randomTextNumber = random+randomlagi+'---------kuhong-api-storage'+'DidinSec';
...
router.get('/flaming', async (req, res, next) => {
  ...
  try {
	 ...
	 var buffer = await (await fetch(`https://api.imgbb.com/1/upload?expiration=120&key=761ea2d5575581057a799d14e9c78e28&image=${json.result}&name=DidinSec`)).json()
